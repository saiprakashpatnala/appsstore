...
class AppStore extends Component {
  state = {item: '', activeTab: tabsList[0].tabId}
  ...
  render() {
    ...
    const activeApps = appsList.filter(app => app.category === activeTab)

    const filteredResults = activeApps.filter(each =>
      each.appName.toLowerCase().includes(item.toLowerCase()),
    )
      ...
      <div className="app-con">
        ...
        <ul className="app-con">
          {filteredResults.map(i => (
            <AppItem appDetails={i} key={i.appId} />
          ))}
        </ul>
      </div>
    )
